...
exports.signUpUsers = async (req, res) => {
  // Define una función asíncrona para agregar usuarios
  try {
...
exports.signInUsers = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ auth: true, token });// Envía la respuesta con los datos del usuario guardado en formato JSON
  } catch (error) {
    res.status(500).json({ message: error.message }); // Si ocurre un error, envía una respuesta con el estado 500 y el mensaje de error
  }
...
exports.listUsers = async (req, res) => {
  // Define una función asíncrona para listar usuarios
  try {
...
exports.listUsersById = async (req, res) => {
  // Define una función asíncrona para listar un usuario por su ID
  try {
...
exports.updateUsers = async (req, res) => {
  // Define una función asíncrona para actualizar usuarios
  try {
    const { id } = req.params; // Extrae el ID de los parámetros de la solicitud
...
exports.deleteUsers = async (req, res) => {
  // Define una función asíncrona para eliminar usuarios
  try {
    const { id } = req.params; // Extrae el ID de los parámetros de la solicitud
